...
<div id="errores2" style=" display: none;" class="row">
    <div class="col-sm-12 col-md-12">
        <div  class="alert alert-danger">
            <label style=" display: block;" id="superacion"><?php echo __('The maximum quota has been reached for this category'); ?></label>
        </div>
    </div>
</div>
        ...
        <div class="row">
            <div class="col-sm-12 col-md-12">
                <div class="alert alert-success">
                    <center>
                        <?php echo __('To cancel all categories, go to'); ?>: &nbsp;&nbsp;<b><a href="http:<?php echo Yii::app()->baseUrl; ?>/usuario/historial">(<?php echo __('History'); ?> -> <?php echo __('Payment History'); ?>)</a></b>
                    </center>
                </div>
            </div>
        </div>
...
<div class="row">
                                        ...
                                        if($competenciaCategoria->idCategoria->id_bloque == $bloque->id_bloque){
                                            if($competenciaCategoria->idCategoria->estatus != 0){
                                                if($model->id_categoria){
                                                    ...
                                                    $idCategoria = $competenciaCategoria->idCategoria->id_categoria;
                                                }
                                                ?>
                ...
                <div class="panel-group" id="accordion">
                    <div class="panel panel-warning">
                      <div class="panel-heading" >
                        ...
                        <div class="row">
                            <div class="col-sm-12 col-md-6">
                                <h4 class="panel-title">
                                    <a data-toggle="collapse" data-parent="#accordion" href="#collapse<?php echo $bloque->id_bloque?>">
                                    <?php echo $model->idCategoria->idBloque->bloque; ?>
                                    </a>
                                </h4>
                            </div>
                            <div style="text-align: right" class="col-sm-12 col-md-6">
                      ...
                      <div id="collapse" class="panel-collapse collapse">
                        <label class="btn btn-default" onclick="" style=" border-top: 0px; border-radius: 0px; width: 100%; height: 35px;">
                            <?php echo $model->idCategoria->categoria."<br>"; ?>
                        </label>
                      </div>
                    </div>
    ...
    if($model->id_inscripcion){
    ?>
        <div id="form_categoria" style=" display: block;" class="col-sm-12 col-md-8">
    <?php
    }else{
    ?>
    ...
    ?>
            ...
            <div class="col-sm-12 col-md-12">
                <div style="" id="mensajesCategoria"> 
                    <div id="errores" style=" display: none;" class="alert alert-danger">
                        <label style=" display: none;" id="superacion"><?php echo __('The maximum quota has been reached for this category'); ?></label>
                        <label style=" display: none;" id="edadFuera"><?php echo __('The selected user does not have the age understood for the selected category'); ?>.</label>
                    </div>
        ...
        <div class="row">
            <div class="col-sm-12 col-md-12">
                ...
                <input type="hidden" id="edadPermitidaMin" name="edadPermitida" value="<?php echo $model->idCategoria->edad_min; ?>">
                <input type="hidden" id="edadPermitidaMax" name="edadPermitida" value="<?php echo $model->idCategoria->edad_max; ?>">
            </div>
        </div>
        <br>
        <div class="row">
...
//                }
        ...
        <div class="row">
            ...
            <div class="col-sm-12 col-md-12">
                <div class="panel panel-warning">
                    <div class="panel-heading">.mp3 =< 6MB </div>
        ...
        <div class="row">
            ...
            <div class="col-sm-12 col-md-4">
                <?php $data = CHtml::listData(Pais::model()->findAll(array('order'=>'pais')), 'codigo', 'pais');
                    echo $form->dropDownListGroup($model,'codigo_pais',array('widgetOptions'=>array('htmlOptions'=>array('class'=>'span5', 'onchange'=>'findCiudad(this.value)'), 'data'=>array(''=>'Seleccione...')+$data)));
                ?>
            </div>
            <div class="col-sm-12 col-md-4">
                <?php 
        ...
        <div class="row">
            <div class="col-sm-12 col-md-12">
                ...
                <div class="panel-heading">
                    ...
                    <h3 class="panel-title" style="display: inline;"><?php echo __('Participants'); ?>: <label id="participante" ></label></h3>
                                            ...
                                            if(isset($participantesCompetenciaCategoria)){
                                               ...
                                               <!--<div class="row">-->
                                                   ...
                                                   <div class="col-xs-4" style="padding: 8px;"> 
                                                       <label><?php echo $participanteCC->idUsuario->cedula;?></label>
                                                   </div>
                                                    <?php 
//                                                    if(!$model->id_inscripcion){
                                                    ?>
...
<!--                    <div class="form-actions">-->
                            ...
                            if($competencia->gratis != 1){
                                ...
                                $this->widget('booster.widgets.TbButton', array(
                                    'buttonType'=>'submit',
                                    'context'=>'primary',
                                    'label'=>$model->isNewRecord ? __('Subscription Category') : __('Update'),
                                )); 
                            }    
                            ?>
                    <!--</div>-->
...
<!--        <div class="row">
    ...
    <center>
        <a href="http:<?php echo Yii::app()->baseUrl; ?>/usuario/historial"><?php echo __('Finish Subscription'); ?></a>
    </center>
</div>
...
<?php 
if($model->id_categoria){
    $cantidadPermitida = $model->idCategoria->id_tipo_categoria;
    $valorCategoria = 0;
    ...
    if($model->id_categoria == 27 || $model->id_categoria == 28 || $model->id_categoria == 30){
        ...
        $competencias = CompetenciaTipo::model()->findAll('id_copetencia ='.$model->id_copetencia.' AND id_tipo_categoria ='.$model->idCategoria->id_tipo_categoria.' ORDER BY fecha DESC');
...
//                echo  "<hr>".$competencia->costo." - ".$competencia->fecha."<br>";
   ...
   ?>
    <input type="hidden" id="cantidadPermitida" name="cantidadPermitida" value="<?php echo $cantidadPermitida; ?>">
    <input type="hidden" id="valorCategoriaController" name="valorCategoriaController" value="<?php echo $valorCategoria; ?>">
    <input type="hidden" id="idCategoria" name="idCategoria" value="<?php echo $model->id_categoria; ?>">
<!--    <input type="text" id="edadPermitidaMax" name="edadPermitidaMax" value="<?php echo $model->idCategoria->edad_max; ?>">
    <input type="text" id="edadPermitidaMin" name="edadPermitidaMin" value="<?php echo $model->idCategoria->edad_min; ?>">-->
   <?php
}
...
//    alert(edadMin+' - '+edadMax);
    ...
    if(categoria){
        if(cantidadP >= 3){
            var idCategoria = $('#categoriaSeleccionadaFinal').val();
            ...
            if(countRow >= cantidadP){  
                $('#UsuarioEncontrado').html('<div class="alert alert-danger"><b><?php echo __('You have reached the limit of allowed participants!'); ?></b></div>');
            }else{
                var idCategoria = $('#categoriaSeleccionadaFinal').val();
                var idCompetencia = $('#id').val();
                var tipo = $('#tipo').val();  
        ...
        alert("<?php echo __('You must select a category'); ?>");
    }
//    alert(idCategoria);
}
