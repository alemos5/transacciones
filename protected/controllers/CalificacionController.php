...
'actions'=>array('create','update', 'orden', 'listadoCategoria','listadoInscripcion', 'listadoGeneral', 'ordenPosicion', 'listadoCategoriaPosicion','listadoInscripcionPosicion'),
'users'=>array('@'),
),
...
if($model->save())
$this->redirect(array('view','id'=>$model->id_calificacion));
}
...
public function actionOrdenPosicion()
{
$dataProvider=new CActiveDataProvider('Calificacion');
$this->render('orden_posicion',array(
'dataProvider'=>$dataProvider,
));
}

public function actionListadoCategoriaPosicion($id_copetencia = Null)
{
    $id_copetencia = $_GET['idc'];
    //$dataProvider=new CActiveDataProvider('Dj');
    $this->render('_listado_categoria_posicion',array(
    'id_copetencia'=>$id_copetencia,
    ));
}
public function actionListadoInscripcionPosicion($id_copetencia = Null, $id_categoria = Null)
{
//    echo var_dump($_GET); die();
    $id_copetencia = $_GET['idc'];
    $id_categoria = $_GET['idca_'];
    //$dataProvider=new CActiveDataProvider('Dj');
    $this->render('_listado_inscripcion_posicion',array(
    'id_copetencia'=>$id_copetencia, 'id_categoria'=>$id_categoria
    ));
}
