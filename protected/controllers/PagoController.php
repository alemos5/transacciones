...
'actions'=>array('create','update', 'pagoPaypal', 'ReportePago', 'listadoPase', 'listadoCategoria', 'validarPago', 'corregirPago', 'corregirPagoCategoria', 'detallePago'),
'users'=>array('@'),
),
...
public function actionDetallePago(){
    $id_usuario = $_POST['id_usuario'];
    
    $pagos = Pago::model()->findAll('id_usuario ='.$id_usuario);
    ?>
    <table class="table table-hover table-bordered">
            <thead>
                <tr style="background-color: #ccc;">
                    <td><strong><center><?php echo __('ID Payment'); ?></center></strong></td>
                    <td><strong><center><?php echo __('Items'); ?></center></strong></td>
                    <td><strong><center><?php echo __('Detail'); ?></center></strong></td>
                    <td><strong><center><?php echo __('Status'); ?></center></strong></td>
                    <td><strong><center><?php echo __('Round'); ?></center></strong></td>
                </tr>
            </thead>
            <?php
            $idInscripcion = 0;
            foreach ($pagos as $pago){
                ...
                if($pago->id_pago_estatus == 2){
                    ...
                    if($pago->id_pago_estatus == 1){
                        $estatus = "For validating";
                    }else{
                        if($pago->id_pago_estatus == 3){
                            $estatus = "Denied";
                        }else{
                            $estatus = "In process";
                        }
                ...
                ?>
                <tr>
                    <td><center><?php echo $pago->id_pago; ?></center></td>
                    <td><center><?php echo $tipoPago; ?></center></td>
                    <td>
                        <center>
                            <?php
                    ...
                    <td>
                        <center>
                            <?php 
                            //echo $idInscripcion->ronda;
                            $inscripcionSelect = Inscripcion::model()->find('id_inscripcion ='.$idInscripcion);
                            $rondas = OrganizacionRonda::model()->findAll('id_competencia = 47');
                            $options = '<option value="">Seleccione</option>';
                            foreach ($rondas as $ronda){
                                
                                if($idInscripcion){
                                    if($inscripcionSelect->ronda == $ronda->id_organizacion_ronda){
                                        $options .= '<option selected="selected" value="'.$ronda->id_organizacion_ronda.'">'.$ronda->ronda.'</option>';
                                    }else{
                                        $options .= '<option value="'.$ronda->id_organizacion_ronda.'">'.$ronda->ronda.'</option>';
                                    }
                                }
                            ...
                            if($idInscripcion != 0){
                                ...
                                ?>
                                <select onchange="actualizarRonda(<?php echo $idInscripcion; ?>, this.value);"  id="ronda" class="span5 form-control" name="ronda" placeholder="Day">
                                   <?php echo $options; ?>
                                </select>
                                <?php
                            }
                    ...
                    <td><center><?php echo $estatus; ?></center></td> 
                </tr>
...
public function actionValidarPago(){
    
    $id_pago = $_POST['id_pago'];
    $accion = $_POST['accion'];
    
    $model=$this->loadModel($id_pago);
    $model->attributes=$_POST['id_pago'];
    $model->id_pago_estatus = $accion;
    $model->id_usuario_registro = 1;
    $model->ip_registro = Usuario::getRealIP();
...
public function actionCorregirPago(){
    $this->render('_corregir_pago',array(
//        'model'=>$this->loadModel($id),
    ));
}

public function actionCorregirPagoCategoria(){
    $this->render('_corregir_pago_categoria',array(
//        'model'=>$this->loadModel($id),
    ));
}
...
//            $paseUsuario = Pago::model()->findAll('id_usuario ='.Yii::app()->user->id_usuario_sistema);
//            if(count($paseUsuario) == 0)
...
public function actionListadoPase()
{
    $this->render('_listado_pase',array(
//    'dataProvider'=>$dataProvider,
    ));
}
public function actionListadoCategoria()
{
...
public function actionReportePago(){
    ...
    if($_GET['idPago'] == 00){
        ...
        $id = $_GET['idPago'];
        $model=$this->loadModel($id);
        $model->attributes=$_GET['idPago'];
        $model->costo_pagado=$_POST['amt'];
        $model->descripcion='Referencia PayPal: '.$_POST['tx'].' - Descripcion del Pago: '.$_POST['item_name'];
        $model->fecha_pagado = date('Y-m-d H:i:s');
        $model->id_pago_estatus = 2;
        $model->save();
    }

    $this->redirect(array('usuario/historial'));
}
