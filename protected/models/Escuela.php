...
class Escuela extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('escuela', 'required'),
			array('estatus', 'numerical', 'integerOnly'=>true),
			array('escuela', 'length', 'max'=>250),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id_escuela, escuela, estatus', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'inscripcions' => array(self::HAS_MANY, 'Inscripcion', 'id_escuela'),
		);
	}
