...
class Bloque extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('bloque', 'required'),
			array('estatus', 'numerical', 'integerOnly'=>true),
			array('bloque', 'length', 'max'=>250),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id_bloque, bloque, estatus', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'categorias' => array(self::HAS_MANY, 'Categoria', 'id_bloque'),
		);
	}
